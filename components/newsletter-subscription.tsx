  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">다음 발표 소식 받기</h2>
        <p className="text-xl text-blue-100">이메일로 다음 모임과 발표 소식을 받아보세요</p>
      </div>

      {state?.success ? (
        <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-6 text-center max-w-md mx-auto">
          <CheckCircle className="h-12 w-12 text-green-500 mx-auto mb-4" />
          <h3 className="text-xl font-semibold text-white mb-2">구독이 완료되었습니다!</h3>
          <p className="text-gray-300">개취뽀의 다음 모임과 발표 소식을 이메일로 받아보실 수 있습니다. 감사합니다!</p>
        </div>
      ) : (
        <form action={formAction} className="flex flex-col md:flex-row gap-4 max-w-md mx-auto">
          ...
          <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white h-12" disabled={isPending}>
            {isPending ? (
              <>
