...
export async function submitSponsorApplication(
  ...
  try {
    ...
    // 필수 필드 검증
    const requiredFields = ["companyName", "contactName", "contactEmail", "contactPhone", "motivation"]
    ...
    if (existingApplication) {
      return { error: "30일 내에 이미 신청하신 회사입니다. 잠시 후 다시 시도해주세요." }
    }
    ...
    if (insertError) {
      ...
      return { error: "데이터베이스 저장 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요." }
    }
    ...
    return { error: "신청 처리 중 예상치 못한 오류가 발생했습니다. 잠시 후 다시 시도해주세요." }
  }
}
